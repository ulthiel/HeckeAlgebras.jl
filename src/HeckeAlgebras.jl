# By M. Albert (2022), based on IHecke by J. Gibson (https://github.com/joelgibson/IHecke)
...
import CoxeterGroups: CoxeterElement, CoxeterGroup

export FreeModuleCox, HeckeAlgebra, AntisphericalModule, SphericalModule, BasisHecke, ElementHecke, :, getindex, -, +, *, ^, Bar, BasisHeckeStd, StandardBasis,
...
export ZZ, LaurentPolynomialRing, CoxeterGroup
...
"""
# Returns a representative of the right descent set of w.
function RepRightDescentSet(w::CoxeterElement)
    W = w.G
    for i = 1:size(W.S)[1]
        s = CoxeterElement(W,[i])
...
function IsMinimal(I::Array{Int,1}, w::CoxeterElement)
    W = w.G
    for i = 1:size(I)[1]
        if length(CoxeterElement(W, [i]) * w) < length(w)
            return false
        end
    end
    return true
...
struct HeckeAlgebra <: FreeModuleCox
    ...
    CoxGroup::CoxeterGroup # Coxeter group of finitely presented type.
end

# Function for creating a new Hecke algebra.
function HeckeAlgebra(W::CoxeterGroup)
...
function AntisphericalModule(W::CoxeterGroup, I::Array{Int, 1})
    A, v = LaurentPolynomialRing(ZZ, "v")
...
function SphericalModule(W::CoxeterGroup, I::Array{Int,1})
    A, v = LaurentPolynomialRing(ZZ, "v")
...
struct ElementHecke
    ...
    Terms::Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}
...
function _ToBasis(A::BasisHecke, B::BasisHecke, w::CoxeterElement)
    return false
end

# Fallback implementation, which uses _ToBasis(::BasisHecke, ::BasisHecke, ::CoxeterElement) if available.
# Bases should override either this, or the CoxeterElement version.
...
function getindex(A::BasisHecke, w::CoxeterElement)
    w.G == A.FreeModule.CoxGroup || throw(ArgumentError("w.G == A.FreeModule.CoxGroup required"))
    terms = Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
...
function Base.show(io::IO, mime::MIME"text/plain", elt::ElementHecke)
    ...
    if elt.Terms == Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
        print("(0)*", symbol, "[id]")
    else
        bool = false
...
function _AddScaled(termsA::Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}, termsB::Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}, scalar::LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}})
...
function _AddScaledTerm(terms::Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}, w::CoxeterElement, scalar::LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}})
    if w in keys(terms)
        terms[w] += scalar
    else
        terms[w] = scalar
...
function +(elt1::ElementHecke, elt2::ElementHecke)
    ...
    assocs = Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
...
function *(eltA::ElementHecke, eltB::ElementHecke)
    ...
    if eltA.Terms == Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}() || eltB.Terms == Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
        return ElementHecke(eltA.Parent, Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}())
    end
...
function ^(elt::ElementHecke, n::Int)
    ...
    if n == 0
        terms = Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
        terms[CoxeterElement(W, [])] = one(elt.Parent.FreeModule.BaseRing)
        return ElementHecke(elt.Parent, terms)
    end
    result = elt
    for k = 1:n-1
...
function _RightMultStdGen(M::BasisHeckeStd, eltM::ElementHecke, s::CoxeterElement)
...
function _RightAction(M::BasisHeckeStd, eltM::ElementHecke, terms::Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}})
    acc = Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
    for w in keys(terms)
        piece = eltM
        for s in w.w
            piece = _RightMultStdGen(M, piece, CoxeterElement(w.G, [s]))
        end
...
function _BarInvolutionStd(M::BasisHeckeStd, w::CoxeterElement)
    if w in keys(M.BarCache)
        return M.BarCache[w]
    end
    if length(w) == 0
        return M[CoxeterElement(w.G, [])]
    end
    v = gen(M.FreeModule.BaseRing)
    s = RepRightDescentSet(w)
...
struct HeckeAlgebraStdBasis <: BasisHeckeStd
    ...
    BarCache::Dict{CoxeterElement, ElementHecke} # A dictionary, used as a cache for the bar involution on basis elements.
...
StandardBasis(HAlg::HeckeAlgebra) = HeckeAlgebraStdBasis(HAlg, "H", Dict{CoxeterElement,ElementHecke}(), Array{Int, 1}(), zero(HAlg.BaseRing))
...
StandardBasis(ASMod::AntisphericalModule) = ASModuleStdBasis(ASMod, "aH", Dict{CoxeterElement, ElementHecke}(), ASMod.Para, -gen(ASMod.BaseRing))
...
StandardBasis(SMod::SphericalModule) = SModuleStdBasis(SMod, "sH", Dict{CoxeterElement, ElementHecke}(), SMod.Para, gen(SMod.BaseRing)^-1)
...
function _RightMultKLGen(elt::ElementHecke, s::CoxeterElement, I::Array{Int, 1}, eig::LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}})
...
function _MuCoeffs(H::BasisHeckeStd, K::BasisHeckeKL, w::CoxeterElement)
    if w in keys(K.MuCache)
        return K.MuCache[w]
    end
    if length(w) == 0
        return Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
    end
    Cw = _ToBasis(H, K, w)
    mu = Dict{CoxeterElement, Nemo.fmpz}()
...
function _KLToStd(H::BasisHeckeStd, K::BasisHeckeKL, w::CoxeterElement, I::Array{Int, 1}, eig::LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}})
    if w in keys(K.KLInStdCache)
        return K.KLInStdCache[w]
    end
    if length(w) == 0
        return H[CoxeterElement(H.FreeModule.CoxGroup, [])]
    end
...
function _StdToKL(H::BasisHeckeStd, K::BasisHeckeKL, w::CoxeterElement)
    if w in keys(K.StdInKLCache)
        return K.StdInKLCache[w]
    end
    W = H.FreeModule.CoxGroup
    if length(w) == 0
        return K[CoxeterElement(W, [])]
    end
    terms = Dict{CoxeterElement, LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}}()
...
struct HeckeAlgebraKLBasis <: BasisHeckeKL
    ...
    KLInStdCache::Dict{CoxeterElement, ElementHecke} # A dictionary, used as a cache for the basis change from Kazhdan-Lusztig basis to standard basis.
    StdInKLCache::Dict{CoxeterElement, ElementHecke} # A dictionary, used as a cache for the basis change from standard basis to Kazhdan-Lusztig basis.
    MuCache::Dict{CoxeterElement, Dict{CoxeterElement, Nemo.fmpz}} # A dictionary, used as a cache for the coefficient mu.
    Para::Array{Int, 1} # Parabolic subset (equals Array{Int, 1}() for the Hecke algebra).
    Eig::LaurentPolyWrap{Nemo.fmpz, Nemo.fmpz_poly, LaurentPolyWrapRing{Nemo.fmpz, Nemo.FmpzPolyRing}}
    # Irrelevant for the full Hecke algebra , -v or v^-1 for the antispherical/spherical modules.
end
...
KazhdanLusztigBasis(HAlg::HeckeAlgebra) = HeckeAlgebraKLBasis(HAlg, "K", Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, Dict{CoxeterElement, Nemo.fmpz}}(), Array{Int, 1}(), zero(HAlg.BaseRing))

# Express K[w] in the standard basis.
function _ToBasis(H::HeckeAlgebraStdBasis, K::HeckeAlgebraKLBasis, w::CoxeterElement)
    return _KLToStd(H, K, w, Array{Int, 1}(), zero(H.FreeModule.BaseRing))
end

# Express H[w] in the Kazhdan-Lusztig basis.
function _ToBasis(K::HeckeAlgebraKLBasis, H::HeckeAlgebraStdBasis, w::CoxeterElement)
...
KazhdanLusztigBasis(ASMod::AntisphericalModule) = ASModuleKLBasis(ASMod, "aK", Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, Dict{CoxeterElement, Nemo.fmpz}}(), ASMod.Para, -gen(ASMod.BaseRing))
...
function _ToBasis(aH::ASModuleStdBasis, aK::ASModuleKLBasis, w::CoxeterElement)
    return _KLToStd(aH, aK, w, aK.Para, aH.Eig)
end

# Express aH[w] in the Kazhdan-Lusztig basis.
function _ToBasis(aK::ASModuleKLBasis, aH::ASModuleStdBasis, w::CoxeterElement)
...
KazhdanLusztigBasis(SMod::SphericalModule) = SModuleKLBasis(SMod, "sK", Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, ElementHecke}(), Dict{CoxeterElement, Dict{CoxeterElement, Nemo.fmpz}}(), SMod.Para, gen(SMod.BaseRing)^-1)
...
function _ToBasis(sH::SModuleStdBasis, sK::SModuleKLBasis, w::CoxeterElement)
    return _KLToStd(sH, sK, w, sK.Para, sH.Eig)
end

# Express sH[w] in the Kazhdan-Lusztig basis.
function _ToBasis(sK::SModuleKLBasis, sH::SModuleStdBasis, w::CoxeterElement)
